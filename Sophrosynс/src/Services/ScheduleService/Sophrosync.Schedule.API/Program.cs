...
using Sophrosync.Schedule.Infrastructure;
...
builder.Host.UseSerilog((ctx, lc) => lc
    ...
    .Enrich.WithProperty("Service", "Schedule"));

builder.Services.AddControllers();
builder.Services.AddEndpointsApiExplorer();
builder.Services.AddSwaggerGen(options =>
    options.SwaggerDoc("v1", new() { Title = "Schedule API", Version = "v1" }));
...
{
    options.AddPolicy("CanManageAppointments", p =>
        p.RequireClaim("roles", "admin", "therapist", "supervisor", "receptionist"));

    options.AddPolicy("CanReadAppointments", p =>
        p.RequireClaim("roles", "admin", "therapist", "supervisor", "receptionist"));
});
...
builder.Services.AddValidatorsFromAssembly(typeof(ScheduleAppointmentCommand).Assembly);
